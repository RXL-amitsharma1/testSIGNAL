...
class TopicListReportBuilderService {
    ...
    void createReport(JRDataSource dataSource, Map params, ArrayList<JasperReportBuilderEntry> jasperReportBuilderEntryList) {
        if (dynamicReportService.isInPrintMode(params)) {
            ...
            jasperReportBuilderEntry.excelSheetName = "Topic Listing"
    ...
    private JasperReportBuilder buildReport(JRDataSource dataSource, Map params) {
        ...
        setPrintablePageHeader(report, "Topic Listing", false, params)
        dynamicReportService.noDataCheck(dataSource, report)
    }

    private void addColumns(JasperReportBuilder report, Map params) {
        def columnField = [[fieldName: "topicName", columnName: "Topic Name"],
                           [fieldName: "productname", columnName: "Product Name"],
                           [fieldName: "noOfPec", columnName: "PEC Count"],
                           [fieldName: "noOfCases", columnName: "Case Count"],
                           [fieldName: "assignedTo", columnName: "Assigned To"],
                           [fieldName: "priority", columnName: "Priority"],
                           [fieldName: "actions", columnName: "Actions"]
        ]
        columnField.each {
            if (it.fieldName == "noOfPec" || it.fieldName == "noOfCases" || it.fieldName == "actions") {
                report.addColumn(Columns.column(it.columnName, it.fieldName, type.integerType()))
            } else {
