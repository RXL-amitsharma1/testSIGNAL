import sys
from src_ja.spam_ham_model import SpamIdentification
import logs
...
def spam_identification(pvi_json, text):
    try:
        logger.info("spam identification for japanese called")
        spam_result = spam_model.predict(text)
        logger.info("spam results: %s", spam_result)
        if spam_result['SPAM'] > SPAM_HAM_THRESHOLD:
            pvi_json["code"] = 4
            pvi_json["message"] = "spam"
            pvi_json["spam_acc"] = spam_result['SPAM']
        return pvi_json
