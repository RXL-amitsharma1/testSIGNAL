        ...
        _onColumnChange: function onColumnChange(columnClass, value, oldValue) {
            ...
            if (this.movingUpOrDown) {
                this.fastMoveChangeFn = function () {
                    that._onColumnChange(columnClass, value, oldValue);
                };
                return;
            }
...
//            this.lastColEl = this.columns()[this.columns().indexOf(column) + 1];
            ...
            if (this.preColEl && this.preColEl.querySelector(".highlighted") && !this.currentColEl.querySelector(".highlighted")) {
                // if change to another search result
                that._resetColumns(this.columns().indexOf(column)); // reset other columns
            }

            var preSelected = that.carriage.querySelector(".selectedBackground");
            if (preSelected) {
                preSelected.classList.remove("selectedBackground");
            }
            that.currentColEl.querySelector(".selected").classList.add("selectedBackground");
            ...
            function appendIfValueIsSame(data, level) {
                ...
                if (that.nextColEl && that.nextColEl.querySelector(".highlighted")) { // in search path
                    ...
                    that._appendCol(newData, level, 1);
                    that.callbacks.source(newData.selectedPath, appendIfValueIsSame, parseInt(level) + 1, that.dictionaryType, false);
                } else if (!that.currentColEl.querySelector(".highlighted")) {
                    that._appendCol(data, level, 1);
                }
            ...
            function showParents(data, level) {
                ...
                if (data && level > 1) {
                    ...
                    if (that.currentColEl.querySelector(".highlighted") && that.preColEl && that.preColEl.querySelector(".highlighted")) {
                        var newData = generateNewDataForCol(data, that.preColEl);
                        that._appendCol(newData, level, -1);
            ...
            function ready() {
                ...
                switch (that.dictionaryType) {
                    case EVENT_DICTIONARY:
                        that._appendCol(emptyData, "SOC");
                        that._appendCol(emptyData, "HLGT");
                        that._appendCol(emptyData, "HLT");
                        that._appendCol(emptyData, "PT");
                        that._appendCol(emptyData, "LLT");
                        that._appendCol(emptyData, "Synonyms");
                        break;
                    case PRODUCT_DICTIONARY:
                        that._appendCol(emptyData, "Ingredient");
                        that._appendCol(emptyData, "Family");
                        that._appendCol(emptyData, "Product Generic Name");
                        that._appendCol(emptyData, "Trade Name");
                        break;
                    case STUDY_DICTIONARY:
                        that._appendCol(emptyData, "StudyNumber");
                        that._appendCol(emptyData, "ProtocolNumber");
                        that._appendCol(emptyData, "Center");
                        break;
                }
                ...
                for (var i = 0; i < inputList.length; ++i) {
                    inputList[i].addEventListener("keyup", function (e) {
                        ...
                        if (key == 13) { // 13 is enter
                            ...
                            clearSearchInputs(searchInput.indexOf(this), that.dictionaryType);
//                            clearAllText(that.dictionaryType);
            ...
            function showSearchResult(data, level) {
                that._appendCol(data, level, 0);
//                that.callbacks.change.call(that, value);
            }
        },

        _appendCol: function appendCol(data, dictionaryLevel, colDiff) {
            var col = this._createCol(data, dictionaryLevel, colDiff);
            if (this.ready) this._alignCols();
//            this.lastColEl = null;
            return col;
        },

        _createCol: function createCol(data, dictionaryLevel, colDiff) {
            var col;
...
//            if (this._selectedEl === el) return;
        ...
        itemTemplate: function itemTemplate(data) {
            return '<li dictionaryLevel="' + data.level + '" data-value="' + data.id + '">' + data.name + '</li>';
        },
