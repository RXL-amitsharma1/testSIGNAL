...
@DirtyCheck
class LiteratureDateRangeInformation extends BaseDateRangeInformation {
    ...
    static mapping = {
        table name: "LITERATURE_DATE_RANGE"
    ...
    String toString() {
        ...
        if(this.dateRangeEnum != DateRangeEnum.CUMULATIVE && this.dateRangeEnum != DateRangeEnum.CUSTOM) {
            DateRangeEnum relativeDateRange = this.dateRangeEnum
            Date nextRunDate = literatureConfiguration?.nextRunDate ?: new Date()
            dateRange = RelativeDateConverter.(relativeDateRange.value())(new java.util.Date(nextRunDate?.getTime()), this.relativeDateRangeValue ?: 1, this.literatureConfiguration?.configSelectedTimeZone)
        } else if (this.dateRangeEnum == DateRangeEnum.CUSTOM) {
