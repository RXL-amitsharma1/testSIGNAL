...
@DirtyCheck
class ExecutedAlertDateRangeInformation extends BaseDateRangeInformation {
    ...
    static belongsTo = [executedAlertConfiguration: ExecutedConfiguration]

    static mapping = {
        table name: "EX_ALERT_DATE_RANGE"
    ...
    def beforeValidate() {
        if (this.dateRangeEndAbsolute && this.dateRangeStartAbsolute) {
            this.dateRangeStartAbsoluteDelta = RelativeDateConverter.getDaysDifference(this.executedAlertConfiguration.nextRunDate, this.dateRangeStartAbsolute)
            this.dateRangeEndAbsoluteDelta = RelativeDateConverter.getDaysDifference(this.executedAlertConfiguration.nextRunDate, this.dateRangeEndAbsolute)
        }
    ...
    List<Date> getReportStartAndEndDate() {
        if (this.dateRangeEnum != DateRangeEnum.CUMULATIVE && this.dateRangeEnum != DateRangeEnum.CUSTOM) {
            ...
            Date nextRunDate = executedAlertConfiguration?.nextRunDate ?: new Date()
            return RelativeDateConverter.(relativeDateRange.value())(new Date(nextRunDate?.getTime()), this.relativeDateRangeValue ?: 1, executedAlertConfiguration?.configSelectedTimeZone)
    ...
    String toString() {
        ...
        if (this.dateRangeEnum != DateRangeEnum.CUMULATIVE && this.dateRangeEnum != DateRangeEnum.CUSTOM) {
            DateRangeEnum relativeDateRange = this.dateRangeEnum
            Date nextRunDate = executedAlertConfiguration?.nextRunDate ?: new Date()
            dateRange =  RelativeDateConverter.(relativeDateRange.value())(new Date(nextRunDate?.getTime()), this.relativeDateRangeValue ?: 1, executedAlertConfiguration?.configSelectedTimeZone)
        } else if (this.dateRangeEnum == DateRangeEnum.CUSTOM) {
