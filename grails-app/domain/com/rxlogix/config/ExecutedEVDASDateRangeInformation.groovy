...
@DirtyCheck
class ExecutedEVDASDateRangeInformation extends BaseDateRangeInformation {
    ...
    static mapping = {
        table name: "EX_EVDAS_DATE_RANGE"
    ...
    static belongsTo = [execEvdasConfiguration: ExecutedEvdasConfiguration]
    ...
    def getReportStartAndEndDate() {
        ...
        if (this.dateRangeEnum != DateRangeEnum.CUMULATIVE && this.dateRangeEnum != DateRangeEnum.CUSTOM) {
            ...
            Date nextRunDate = execEvdasConfiguration?.nextRunDate ?: new Date()
            return RelativeDateConverter.(relativeDateRange.value())(new java.util.Date(nextRunDate?.getTime()), this.relativeDateRangeValue ?: 1, this.execEvdasConfiguration?.configSelectedTimeZone)
    ...
    def beforeValidate() {
        if (this.dateRangeEndAbsolute && this.dateRangeStartAbsolute) {
            this.dateRangeStartAbsoluteDelta = RelativeDateConverter.getDaysDifference(this.execEvdasConfiguration?.nextRunDate, this.dateRangeStartAbsolute)
            this.dateRangeEndAbsoluteDelta = RelativeDateConverter.getDaysDifference(this.execEvdasConfiguration?.nextRunDate, this.dateRangeEndAbsolute)
        }
    ...
    String toString() {
        ...
        if (this.dateRangeEnum != DateRangeEnum.CUMULATIVE && this.dateRangeEnum != DateRangeEnum.CUSTOM) {
            DateRangeEnum relativeDateRange = this.dateRangeEnum
            Date nextRunDate = execEvdasConfiguration?.nextRunDate ?: new Date()
            dateRange =  RelativeDateConverter.(relativeDateRange.value())(new Date(nextRunDate?.getTime()), this.relativeDateRangeValue ?: 1, execEvdasConfiguration?.configSelectedTimeZone)
        } else if (this.dateRangeEnum == DateRangeEnum.CUSTOM) {
