...
<html>
<head>
    <title>Document Validation Reminder</title>
</head>

<body>
<h2>Message:</h2>

<div><g:message code='app.document.validation.reminder.msg'/></div>
...
<div>
    <ul>
        ...
        <li>Target completion date: ${com.rxlogix.util.DateUtil.toUSDateString(
                document?.targetDate, timezone)}</li>
        <li>Document status: ${document?.documentStatus}</li>
        <li>Document status date: ${com.rxlogix.util.DateUtil.toUSDateString(
                document?.statusDate, timezone)}</li>
