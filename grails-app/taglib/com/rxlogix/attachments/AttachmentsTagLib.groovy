...
class AttachmentsTagLib {
    ...
    def each = { attrs, body ->
        ...
        if (AttachmentableUtil.isAttachmentable(bean)) {
            ...
            if (inputNames) {
                ...
                (bean.class != ValidatedSignal) ? [] : attachments.add(bean.getReferences())
            } else {
                attachments = bean.attachments
                (bean.class != ValidatedSignal) ? [] : attachments.add(bean.getReferences())
            }

            attachments?.flatten()?.eachWithIndex { attachment, idx ->
    ...
    def style = { attrs ->
        out << g.render( template: '/attachments/resources/style')
    }
    ...
    def script = { attrs ->
        ...
        out << g.render(
                template: '/attachments/resources/script',
    ...
    def icon = { attrs ->
        ...
        out << "<img width=\"16\" height=\"16\" src=\"" + g.resource(dir: 'images/silk', file: FILE_ICON_MAP[bean.ext?.toLowerCase()] ?: 'page_white') + ".png\" alt=\"${bean.ext}\"/>"
    }
